"""
Day 29 - Custom Road Damage Detection Application (Streamlit).

Upload a road photo or a short video, run our custom-trained YOLO model
(trained on the Roboflow "Road Damage Dataset"), see every detection's
bounding box (a distinct colour per class), class name and confidence
score, and download the annotated result.
...
st.set_page_config(page_title="Road Damage Detection (Custom YOLO)", page_icon="\U0001F6E3️", layout="wide")
...
@st.cache_resource(show_spinner="Loading the trained road-damage model...")
def get_model():
...
st.title("\U0001F6E3️ Custom Road Damage Detection")

if not WEIGHTS_PATH.exists():
    st.error(
        "best.pt not found next to app.py. Train a model first "
        "(see train.py / README.md), then place best.pt here."
    )
    st.stop()
...
st.markdown(
    f"Detects road surface damage - **{', '.join(info['classes'])}** - in a photo or "
    "video using a **custom YOLOv8 model we trained** on the Roboflow "
    "[Road Damage Dataset](https://universe.roboflow.com/road-damage-detection-ds22n/road-damage-dataset-8jvz5) "
    "(not a pretrained COCO checkpoint - these classes don't exist in COCO)."
)

with st.sidebar:
    st.header("Model")
    st.caption(f"{info['class_count']} classes, {info['param_count']:,} parameters")
...
if mode == "Image":
    ...
    cols[1].metric("Damage instances detected", len(result.detections))
    ...
    with tab_result:
        ...
        col_b.image(bgr_to_rgb(result.annotated), use_container_width=True,
                    caption=f"Detected (conf >= {conf:.2f})")
    ...
    with tab_gallery:
        st.caption("Runs the same model over every bundled sample image, so you can see class/colour "
                  "consistency across many photos at once.")
...
# ---------------------------------------------------------------------------
    ...
    if st.button("Run detection on this video", type="primary"):
        ...
        with st.spinner("Running the model on every frame..."):
        ...
        st.caption("Class counts across all processed frames (the same pothole/crack is counted once "
                  "per frame it appears in, not once per unique defect):")
        if result.class_counts:
            st.bar_chart(result.class_counts)
        else:
            st.warning("Nothing detected at this confidence threshold - try lowering it in the sidebar.")
...
st.divider()
st.caption("Day 29 - custom YOLOv8 trained on the Roboflow Road Damage Dataset (Ultralytics). "
          "Bounding-box colour is deterministic per class. The banner burned into each result "
          "shows the confidence threshold used, detection count, and inference time.")
