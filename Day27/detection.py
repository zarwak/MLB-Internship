"""
Day 27 - YOLO object detection core module.

Wraps a pretrained Ultralytics YOLO model for image/video inference, with
custom box drawing (not YOLO's built-in .plot()) so we get full control over
one requirement from the brief: a different, consistent bounding-box colour
per class, with labels/boxes/confidence-threshold that stay readable at any
image size. Both the Streamlit app and the coding_practice scripts import
this module - it holds all the actual logic, nothing lives only in app.py.
"""
...
def load_model(name: str = DEFAULT_MODEL) -> YOLO:
    """Load (and cache) a pretrained YOLO model. Downloads weights on first use.
...
def class_color(class_id: int) -> tuple[int, int, int]:
    ...
    so consecutive class ids (which often co-occur, e.g. car/truck/bus) don't
...
# ---------------------------------------------------------------------------
# CLI (mirrors segmentation.py / feature_matching.py in earlier days)
# ---------------------------------------------------------------------------
