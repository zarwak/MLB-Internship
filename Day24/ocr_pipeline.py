...
ENGINES = ("easyocr", "paddleocr", "tesseract", "doctr", "rapidocr")
...
ENGINE_LABELS = {
    ...
    "rapidocr": "RapidOCR - runs on ONNXRuntime instead of a full deep-learning framework, light and fast on CPU",
}
...
# Day 24: multiple documents can now be processed at once via a thread
# pool, and every worker thread that picked the same engine shares that
# one cached reader above. The two engines with an actual documented
# thread-safety guarantee run inference directly, from whichever worker
# thread picked them: onnxruntime's CPU execution provider (what RapidOCR
# runs on) is confirmed safe for concurrent Run() calls on one session by
# Microsoft's own docs, and Tesseract shells out to a brand new OS
# process per call, so there is no in-process state to touch at all.
THREAD_SAFE_ENGINES = {"rapidocr", "tesseract"}
# EasyOCR, PaddleOCR and docTR have no such guarantee published anywhere,
# so their inference calls are routed onto one dedicated single-worker
# thread per engine instead of running on whichever pool worker happens
# to pick them up. A plain threading.Lock() was tried first - it does
# stop two calls overlapping in time, but it does NOT stop DIFFERENT
# threads from taking turns calling in, and that alone was still enough
# to crash PaddleOCR intermittently ("could not execute a primitive")
# even with every call fully serialized and never actually concurrent.
# oneDNN, PaddlePaddle's CPU backend, caches compiled primitives keyed to
# whichever OS thread first built them, and errors when a different
# thread's call later touches that same cached primitive - a lock alone
# does not fix that, because a lock's whole job is letting different
# threads take turns, which is exactly the part that was unsafe. Pinning
# the call to always run on the SAME one thread eliminates the class of
# bug regardless of which pool worker submitted the job. Built once at
# import time, before any worker thread exists.
_reader_executors = {
    engine: concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"{engine}-infer")
    for engine in ENGINES if engine not in THREAD_SAFE_ENGINES
}


def run_on_reader_thread(routing_engine: str, fn, *args, **kwargs):
    """Runs fn() on the one dedicated thread for this engine, or directly
    if the engine is documented safe to call from any thread.

    Named routing_engine, not engine, so it never collides with an
    engine= keyword argument meant for fn - Python matches keyword
    arguments against a function's own parameter names before anything
    reaches **kwargs, so a plain `engine` here would raise "got multiple
    values for argument 'engine'" the moment fn also takes one.
    """
    if routing_engine in THREAD_SAFE_ENGINES:
        return fn(*args, **kwargs)
    return _reader_executors[routing_engine].submit(fn, *args, **kwargs).result()
...
def get_reader(engine: str = "easyocr", languages=("en",)):
    ...
    if engine not in _readers:
        if engine == "easyocr":
            ...
            # DOCTR_CACHE_DIR alone was not the whole story - docTR's own
            # dependency on huggingface_hub downloads some files through
            # HF's own cache, which ignores DOCTR_CACHE_DIR entirely and
            # defaults to ~/.cache/huggingface on C: regardless. Found the
            # hard way on Day 24, when C: quietly filled to 0 bytes free
            # again months after Day 22's original discovery of the same
            # problem - HF_HOME is the variable that actually redirects it.
            ...
            # RapidOCR runs PP-OCR-family models through ONNXRuntime rather
            # than a full deep-learning framework, so - unlike every other
            # engine here - it needs no torch/paddle equivalent at all, just
            # the ~30MB of ONNX model files themselves. Those still default
            # to a home-folder cache the same way EasyOCR/PaddleOCR/docTR's
            # did (see MODEL_CACHE_DIR above), so model_root_dir points it
            # at D: from the start rather than finding that out the hard
            # way. lang_type defaults to "ch" (Chinese) for both detection
            # and recognition - harmless for English text since the model
            # still covers Latin script, but explicit is better than
            # accidentally shipping a model tuned for a language this app
            # never asks for.
            from rapidocr import RapidOCR
            _readers[engine] = RapidOCR(params={
...
def extract_text(reader, image_array: np.ndarray, engine: str = "easyocr",
    ...
    if engine == "easyocr":
        ...
        # RapidOCR takes a numpy array directly and already hands back
        # line-level text and per-line scores as parallel tuples - no
        # BGR/PIL conversion and no manual word-to-line grouping needed,
        # simpler than every other engine here. txts/scores come back as
        # None rather than an empty tuple when nothing is detected, same
        # "guard for it explicitly" lesson as PaddleOCR's raw[0] above.
        result = reader(image_array)
        lines = list(zip(result.txts or (), result.scores or ()))
...
def run_ocr(image: Image.Image, preset: str = "light", straighten: bool = False,
    ...
    # Timed from here, not from get_reader() above - the very first call to
    # an engine also downloads/loads its model, which the app already shows
    # its own "Loading the OCR model" spinner for. Folding that one-time
    # cost into "extraction time" would make the first document of a
    # session look artificially slow next to every one after it.
    started = time.perf_counter()
    processed, angle = preprocess(image, preset=preset, straighten=straighten)
    # Routed onto this engine's one dedicated thread (or run directly, for
    # the two engines confirmed safe from any thread) - see
    # run_on_reader_thread() above. Only the inference call itself moves;
    # preprocessing above already finished on this thread's own private
    # array and stays right where it was.
    result = run_on_reader_thread(engine, extract_text, reader, processed, engine=engine, preset=preset)
    result.elapsed_seconds = time.perf_counter() - started
    return result, processed, angle
...
def run_ocr_auto(image: Image.Image, straighten: bool = False, engine: str = "easyocr",
    ...
    # Overwrite the winning candidate's own elapsed_seconds with the total
    # time Auto mode actually took - it tried two or three presets to get
    # here, and that is the number the person waiting for it experienced,
    # not just the winning candidate's individual run time.
